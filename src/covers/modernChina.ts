...
export const MODERN_CHINA_COVERS: Record<string, string> = {
  ...
  /* ── Eurasian Crossroads · Xinjiang — Taklamakan Desert at dusk, camel caravan, Tian Shan peaks ── */
  eurasianxroads: scene(`
    <defs>
      <linearGradient id="exrSky" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0" stop-color="#0e1828"/><stop offset=".34" stop-color="#182838"/>
        <stop offset=".62" stop-color="#604828"/><stop offset=".82" stop-color="#c08040"/>
        <stop offset="1" stop-color="#d0a060"/>
      </linearGradient>
      <radialGradient id="exrSun" cx="78%" cy="62%" r="30%">
        <stop offset="0" stop-color="#ffb840" stop-opacity=".75"/>
        <stop offset="1" stop-color="#ffb840" stop-opacity="0"/>
      </radialGradient>
    </defs>
    <rect width="300" height="450" fill="url(#exrSky)"/>
    ${stars(10, 411)}
    <rect width="300" height="450" fill="url(#exrSun)"/>
    <circle cx="234" cy="278" r="18" fill="#ffc840" opacity=".82"/>
    <path d="M0 290 Q40 256 80 268 Q110 278 130 260 Q158 240 186 256 Q220 274 260 250 Q280 238 300 244 L300 450 L0 450 Z" fill="#b09060" opacity=".7"/>
    <path d="M0 340 Q100 322 200 334 Q250 342 300 326 L300 450 L0 450 Z" fill="#c8a858"/>
    <path d="M0 390 Q150 378 300 388 L300 450 L0 450 Z" fill="#b89040"/>
    <path d="M0 430 Q150 422 300 428 L300 450 L0 450 Z" fill="#a08030"/>
    ${Array.from({length:12}).map((_,i)=>{const x=10+(i*26)%290; return `<path d="M${x} ${340} Q${x+4} ${320} ${x+8} ${312} Q${x+12} ${320} ${x+16} ${340}" stroke="#c8a858" stroke-width=".5" fill="#b89040" opacity=".5"/>`;}).join('')}
    <g fill="#281c08" opacity=".85">
      ${[58,92,126].map((x,i)=>{const h=32+i%2*10; return `
        <ellipse cx="${x}" cy="${330}" rx="${10+i}" ry="${h}"/>
        <ellipse cx="${x}" cy="${330-h}" rx="${14+i}" ry="${10}"/>
        <rect x="${x-2}" y="${330-h}" width="4" height="${h+8}"/>
        <ellipse cx="${x+14}" cy="${328}" rx="${7+i}" ry="${22+i%2*6}"/>
        <ellipse cx="${x+14}" cy="${328-22-i%2*6}" rx="${10+i}" ry="${8}"/>
        <rect x="${x+12}" y="${328-22-i%2*6}" width="4" height="${30+i%2*6}"/>
      `;}).join('')}
    </g>
    <g fill="#201808" opacity=".7">
      <path d="M0 290 Q40 256 80 268 Q110 278 130 260 Q158 240 186 256 Q220 274 260 250 Q280 238 300 244 L300 260 Q280 252 260 264 Q220 288 186 272 Q158 256 130 274 Q110 292 80 282 Q40 270 0 304 Z"/>
    </g>
    <g fill="#c09050" opacity=".4">
      <rect x="168" y="236" width="30" height="42"/>
      <path d="M162 240 L183 218 L204 240 Z"/>
      <rect x="176" y="218" width="14" height="24"/>
    </g>
    ${Array.from({length:5}).map((_,i)=>`<path d="M${160+i*6} ${278} Q${163+i*6} ${268} ${166+i*6} ${260}" stroke="#d09040" stroke-width=".8" fill="none" opacity=".35"/>`).join('')}
    <g fill="#180e04">
      <circle cx="150" cy="282" r="5.5"/>
      <rect x="147" y="287" width="5" height="8" rx="1"/>
    </g>
    <path d="M0 290 Q150 284 300 288" stroke="#c08040" stroke-width=".5" fill="none" opacity=".35"/>
  `),
};
